...
- **Service `rc-backend`:**
  ...
  - Bind port strictly to host loopback: `127.0.0.1:5000:5000`.
  - Depends on `rc-db` with `condition: service_healthy`.
  - Enforce deploy memory limit: `256M`, reservation: `128M`, CPU limit: `0.50`.
  - Healthcheck: `wget -qO- http://127.0.0.1:5000/api/garage/health || exit 1`.
- **Service `rc-frontend`:**
  - Build context `./frontend`.
  - Bind port strictly to host loopback: `127.0.0.1:3000:3000`.
...
- Reverse proxy mapping:
  - `/api/garage/auth/` -> `http://127.0.0.1:5000` (burst 5, nodelay).
  - `/api/` -> `http://127.0.0.1:5000` (burst 20, nodelay).
  - `/` -> `http://127.0.0.1:3000`.
...
4. Backend healthcheck returns `200 OK` on `http://127.0.0.1:5000/api/garage/health`.
