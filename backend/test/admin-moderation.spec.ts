...
export async function runAdminModerationVerification(
  ...
  try {
    ...
    const outlawAuth: AuthTokenResponse = outlawRegRes.body.data;
    ...
    const overview: AdminOverview = adminOverviewRes.body.data;
    ...
    const vehicle: VehicleEntity = vehRes.body.data;
    ...
    const setup: SetupEntity = setupRes.body.data;
    ...
    assert(
      feedData.items.some((i: FeedItem) => i.id === setup.id),
    ...
    const modAuth: AuthTokenResponse = modRegRes.body.data;
    ...
    const modSummary: AdminUserSummary = promoteRes.body.data;
    ...
    const hiddenSummary: AdminSetupSummary = hideRes.body.data;
    ...
    const feedItemsAfter: FeedItem[] = feedAfterHide.body.data.items;
    assert(
      !feedItemsAfter.some((i) => i.id === setup.id),
    ...
    const adminSetups: PaginatedAdminSetups = adminSetupsRes.body.data;
    assert(
      adminSetups.items.some((s) => s.id === setup.id),
    ...
    const suspendedUser: AdminUserSummary = suspendRes.body.data;
    ...
    const reinstatedUser: AdminUserSummary = reinstateRes.body.data;
    ...
    const auditLog: PaginatedAuditLog = auditRes.body.data;
    assert(auditLog.items.length >= 4, 'Audit log must record all mutations');

    const actions = auditLog.items.map((i) => i.action);
...
async function main(): Promise<void> {
  ...
  if (!process.env.JWT_SECRET) {
    process.env.JWT_SECRET = 'milestone13-e2e-secret';
  }
  ...
  try {
    ...
    await runAdminModerationVerification(baseUrl, database);
  } finally {
    await app.close();
  }
}
